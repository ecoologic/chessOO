...
RSpec.shared_examples :lets do |_parameter|
  ...
  let(:move) { Move.new(board, start_tile, destination_tile) }
  let(:valid_move) { Move.new(board, start_tile, valid_destination_tile) }
  let(:invalid_move) { Move.new(board, start_tile, invalid_destination_tile) }
